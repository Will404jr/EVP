...
const AZURE_AD_TENANT_ID = process.env.AZURE_AD_TENANT_ID || "708f7b5b-20fc-4bc8-9150-b1015a308b9c"
...
const ADMIN_EMAILS = ["esserumaga@nssfug.org", "aabala@nssfug.org"]
  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.redirect(
        `${baseUrl}/?error=azure_ad_error&details=${encodeURIComponent(errorDescription || error)}`,
      )
    }
    ...
    const baseUrl = getBaseUrlFromRequest(req)
    const redirectUri = process.env.REDIRECT_URI || `${baseUrl}/api/auth/callback`

    console.log("Exchanging code for token with redirect URI:", redirectUri)
    const tokenResponse = await getTokenFromCode(code, redirectUri)

    if (!tokenResponse || tokenResponse.error) {
      console.error("Token exchange failed:", tokenResponse?.error_description || "Unknown error")
      return NextResponse.redirect(
        `${baseUrl}/?error=token_exchange_failed&details=${encodeURIComponent(JSON.stringify(tokenResponse))}`,
      )
    }
    ...
    return NextResponse.redirect(`${baseUrl}/?error=auth_callback_error&details=${encodeURIComponent(errorMessage)}`)
  }
}
